...
type CreateReportRequest struct {
    ...
    ReporterRole         string  `json:"reporter_role" validate:"required"`
    Village              string  `json:"village" validate:"required"`
    District             string  `json:"district" validate:"required"`
    BuildingName         string  `json:"building_name" validate:"required"`
    BuildingType         string  `json:"building_type" validate:"required"`
    ReportStatus         string  `json:"report_status" validate:"required"`
    FundingSource        string  `json:"funding_source" validate:"required"`
    LastYearConstruction int     `json:"last_year_construction" validate:"required,min=1900,max=2100"`
    FullAddress          string  `json:"full_address" validate:"required"`
    Latitude             float64 `json:"latitude" validate:"required,min=-90,max=90"`
    Longitude            float64 `json:"longitude" validate:"required,min=-180,max=180"`
    FloorArea            float64 `json:"floor_area" validate:"required,min=0"`
    FloorCount           int     `json:"floor_count" validate:"required,min=1"`
